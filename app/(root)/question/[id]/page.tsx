...
const Page = async ({ params, searchParams }: URLProps) => {
  try {
    ...
    // Added error handling for when getQuestionById fails
    let result;
    try {
      result = await getQuestionById({ questionId: id });
      ...
      return (
        <div className="flex flex-col items-center justify-center min-h-screen">
          ...
          <p className="text-muted-foreground mt-2">
            We encountered an error while loading this question. Please try
            again later.
          </p>
          <Link href="/" className="mt-4 text-primary-500 hover:underline">
            Return to home page
    ...
    const engagementScore =
      (result?.views || 0) +
      (result?.upvotes?.length || 0) * 5 +
      (result?.answers?.length || 0) * 10;
    ...
    const getQuestionLevel = (score: number) => {
      ...
      if (score > 500)
        return { level: "Hot", icon: <Flame className="text-orange-500" /> };
      if (score > 200)
        return { level: "Popular", icon: <Zap className="text-blue-500" /> };
      if (score > 100)
        return {
          level: "Rising",
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                ...
                >
                  <div className="relative">
                    <Image
                      src={
                        result?.author?.picture ?? "/assets/icons/avatar.svg"
                      }
              ...
              <div className="flex flex-wrap gap-6 mt-4">
                ...
                <div className="flex items-center gap-2 text-white/80">
                  ...
                  <span>
                    {formatAndDivideNumber(result?.answers?.length || 0)}{" "}
                    answers
                  </span>
                </div>
                <div className="flex items-center gap-2 text-white/80">
                  ...
                  <span>
                    {formatAndDivideNumber(result?.upvotes?.length || 0)}{" "}
                    upvotes
                  </span>
                </div>
                <div className="flex items-center gap-2 text-white/80">
              ...
              <CardContent className="p-8">
                <div className="flex justify-between items-start mb-6">
                  ...
                  <Votes
                    ...
                    hasAlreadyDownvoted={result.downvotes.includes(
                      mongoUser?._id
                    )}
            ...
            <Card className="overflow-hidden border-none shadow-lg bg-light-900 dark:bg-dark-200 relative">
              ...
              <CardContent className="p-6">
                ...
                <div className="relative pt-5">
                  <div className="w-full h-4 bg-light-800 dark:bg-dark-300 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(engagementScore / 10, 100)}%`,
                      }}
                    ></div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4 mt-8">
                  <div className="bg-light-800/50 dark:bg-dark-300/50 rounded-lg p-4 text-center transform hover:scale-105 transition-transform">
                    <p className="text-2xl font-bold text-primary-500">
                      {formatAndDivideNumber(result?.views)}
                    </p>
                  ...
                  <div className="bg-light-800/50 dark:bg-dark-300/50 rounded-lg p-4 text-center transform hover:scale-105 transition-transform">
                    <p className="text-2xl font-bold text-red-500">
                      {formatAndDivideNumber(result?.downvotes?.length || 0)}
                    </p>
                ...
                <div className="mt-6 pt-6 border-t border-gray-100 dark:border-gray-800">
                  <p className="text-sm text-dark500_light700 mb-3">
                    Asked {getTimestamp(result?.createdAt)}
                  </p>
                ...
                <div className="space-y-3">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <div key={i} className="flex gap-2 items-start group">
                      ...
                      <Link
                        href="#"
                        className="text-sm text-dark400_light700 hover:text-primary-500 transition-colors line-clamp-2 group-hover:translate-x-1"
                      >
                        How to implement authentication in a Next.js application
                        with Clerk?
                      </Link>
                    </div>
                  ))}
